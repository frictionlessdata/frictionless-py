...
def test_report():
    ...
    if platform.type != "windows":
        assert report.task.stats.get("bytes") == 30
    assert report.task.stats.get("fields") == 2
    assert report.task.stats.get("rows") == 2
    if platform.type != "windows":
        assert report.task.stats.get("md5") == "6c2c61dd9b0e9c6876139a449ed87933"
        assert (
            report.task.stats.get("sha256")
