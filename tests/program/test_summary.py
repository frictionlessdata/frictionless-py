...
def test_program_summary_yaml():
    result = runner.invoke(program, "summary data/countries.yaml")
    assert result.exit_code == 1
    assert result.stdout.count("invalid")
    assert result.stdout.count("Describe")
    assert result.stdout.count("Extract")
...
def test_program_summary_json():
    result = runner.invoke(program, "summary data/countries.json")
    assert result.exit_code == 1
    assert result.stdout.count("invalid")
    assert result.stdout.count("Describe")
    assert result.stdout.count("Extract")
...
def test_program_summary_describe_with_required_field():
    result = runner.invoke(program, "summary data/countries.yaml")
    assert result.exit_code == 1
    with open(
