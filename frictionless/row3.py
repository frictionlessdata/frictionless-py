...
class Row3(list):
    ...
    def __init__(
        ...
        *,
        ...
        self.__data = RowData(self, field_map=field_map)
    ...
    def process(self, key=None):
        ...
        for field_name in [key] if key else self.__schema.field_names:
            ...
            # Set/return value
            dict.__setitem__(self.__data, field.name, target)
            if key:
                return target
...
class RowData(dict):
    def __init(self, row, *, field_map):
        self.__row = row
        self.__field_map = field_map

    def __missing__(self, key):
        return self.__row.process(key)
    ...
    def __iter__(self):
        return iter(self.__row.schema.field_names)

    def __len__(self):
        return len(self.__row.schema.fields)
    ...
    def __setitem__(self, key, value):
        ...
        self.__row[field_number] = value
        dict.__setitem__(self, key, self.__row.process(key))
